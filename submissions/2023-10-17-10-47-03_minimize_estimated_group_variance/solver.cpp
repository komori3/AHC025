...
struct Solver {
    ...
    std::vector<std::vector<int>> initialize_groups(const std::vector<Items>& items_sorted) const {
        // 蛇腹　降順
    ...
    std::vector<std::vector<int>> initialize_groups_2(const std::vector<Items>& items_sorted, Xorshift& rnd) {
        const int K = items_sorted.size();
        ...
        std::vector<std::vector<int>> groups(D);
        for (int i = 0; i < K; i++) {
            groups[ds[i]].push_back(items_sorted[i].id);
        }
        return groups;
    }
    ...
    int solve(int algorithm_type) {
        ...
        Xorshift rnd;
        //auto groups = initialize_groups(items_sorted);
        auto groups = initialize_groups_2(items_sorted, rnd);
