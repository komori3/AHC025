...
struct Solver {
    ...
    std::vector<double> calc_means(int K, int num_trial) {
        ...
        for (int i = 0; i < K; i++) {
            means[i] = sum_ws[i] / num_trial;
        }
        return means;
    }

    std::vector<std::vector<int>> initialize_groups(const std::vector<BlobPtr>& blobs_sorted, Xorshift& rnd) {
        const int K = blobs_sorted.size();
        const auto ws = calc_means(K, 10000); // 重さを平均で推定 昇順
        const auto mean = std::accumulate(ws.begin(), ws.end(), 0.0) / D;

        // 総和固定なら、分散の最小化は二乗和の最小化と同義
        std::vector<int> ds(K);
        std::vector<double> sum(D);
        double cost = 0;
        ...
        constexpr int num_loop = 5000000;
        int reject = 0;
        for (int loop = 0; loop < num_loop; loop++) {
            ...
            if (type == 0) {
                ...
                double spd = sum[pd], nspd = spd - ws[i], snd = sum[nd], nsnd = snd + ws[i];
                double diff = nspd * nspd + nsnd * nsnd - spd * spd - snd * snd;
                if (diff <= 0) {
                    reject = 0;
                    ds[i] = nd;
                    sum[pd] = nspd;
                ...
                int di = ds[i], dj = ds[j];
                double sdi = sum[di], nsdi = sdi + ws[j] - ws[i], sdj = sum[dj], nsdj = sdj + ws[i] - ws[j];
                double diff = nsdi * nsdi + nsdj * nsdj - sdi * sdi - sdj * sdj;
                if (diff <= 0) {
                    reject = 0;
                    std::swap(ds[i], ds[j]);
                    sum[di] = nsdi;
        ...
        judge->comment(format("loop=%8d, cost=%.2f", num_loop, cost / D - mean * mean));

        std::vector<std::vector<int>> groups(D);
        for (int i = 0; i < K; i++) {
            groups[ds[i]].push_back(blobs_sorted[i]->id);
        }
        return groups;
    }

    int solve(int algorithm_type) {
        ...
        if (algorithm_type < 2) {
            ...
            auto groups = initialize_groups(blobs, rnd);

            auto group_to_blob = [&](int gid) {
                auto blob = std::make_shared<Blob>(gid);
                for (int k : groups[gid]) {
                    for (int i : blobs[k]->items) {
                        blob->items.push_back(i);
                    }
                }
                return blob;
            };
            ...
            auto get_min_id = [&](int gid) {
                ...
                for (int k = 0; k < K; k++) {
                    if (std::count(group.begin(), group.end(), blobs[k]->id)) {
                        return blobs[k]->id;
                    }
                }
                assert(false);
            ...
            if (algorithm_type == 0) {
                ...
                while (judge->turn < Q) {
                    ...
                    auto res = judge->query(group_to_blob(from)->items, group_to_blob(to)->items);
                    if (res == '<') {
                        // to から最小要素を from へ移動
                        if (groups[to].size() > 1) {
                ...
                // グループ全てをソートして、最大グループから最小グループへ最小要素を移動
                while (judge->turn + NFordJohnson::cap[D] <= Q) {
                    std::vector<BlobPtr> gblobs;
                    for (int gid = 0; gid < D; gid++) {
                        gblobs.push_back(group_to_blob(gid));
                    }
                    auto gblobs_sorted = NFordJohnson::merge_insertion_sort(judge, gblobs);
                    std::vector<std::vector<int>> ngroups(D);
                    for (int i = 0; i < D; i++) {
                        ngroups[i] = groups[gblobs_sorted[i]->id];
                    }
                    groups = ngroups;
